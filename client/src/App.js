...
//         const response = await axios.get(`http://localhost:5000/api/verify-claims`, { 
//           params: { claims: claim } 
//         });
//         // Extract the count from the API response
//         const backingCount = response.data?.esearchresult?.count || "0";
//         return { claim, count: backingCount, result: response.data };
//       })
//     );
//     setVerifiedClaims(results);
//     console.log(results);
//   } catch (error) {
...
//           disabled={verificationLoading || claims.length === 0} 
//           className="button"
//         >
...
//           ) : (
//             verifiedClaims.map((item, index) => (
//               <div key={index}>
//                 <h3>{item.claim}</h3>
//                 {parseInt(item.count) > 0 ? (
//                   <p>Scientific backing found: {item.count} articles found.</p>,
//                   <pre>{JSON.stringify(item.result, null, 2)}</pre>
//                 ) : (
//                   <p>No scientific backing found.</p>
//                 )}
//                 {/* <pre>{JSON.stringify(item.result, null, 2)}</pre> */}
//               </div>
//             ))
...
function App() {
  ...
  // Extract and verify claims in one request
